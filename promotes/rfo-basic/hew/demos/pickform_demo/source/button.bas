...
fn.def button_make (aux$, t$, style,x,y)	% constructor
				% depends on gr.text.height,typeface
				% style:0=link 1=rect 2=oval 3=rounded
				% 4=rounded2 5=bitmap
				% x,y is the top left corner (incl.border)
 ...
 lpad = int(th/10)		% border line thickness
 ...
 l = int(x)
 if is_in ("R",aux$) then l = int(l-mw-2*xpad-1)	% right justify
 if is_in ("C",aux$) then l = int(l-(mw+1)/2-xpad)	% centre
 t = int(y)
 if is_in ("B",aux$) then t = t-bh		% bottom justify (exclusive)
 if is_in ("V",aux$) then t = ceil(y-bh/2)	% vertically centered
 r = l+bw-1
 b = t+bh-1
 ...
 if style=1 then			% rectangle
	gr.rect o,l,t,r+1,b+1		% (inclusive)
	list.add border, o
 endif
 ...
 if style=4 then			% rounded button2
	...
	gr.rect o5, l+rd, t, r-rd, b+1	%(inclusive)
	gr.rect o6, l, t+rd, r+1, b-rd
...
button_skip:
 ...
 call theme_color ("p_btn_backg")	% background
 gr.paint.get p_backg

 if style=1 then
	gr.rect o,l+lpad,t+lpad,r-lpad+1,b-lpad+1	%(inclusive)
	list.add press, o
 endif
 if style=2 then
 ...
 if style=4 then
	...
	gr.rect o5, l+rd, t+lpad, r-rd, b-lpad+1
	gr.rect o6, l+lpad, t+rd, r-lpad+1, b-rd
...
button_flash:				% press the button t$ times
	...
	for f = 1 to n
		if pressed then gosub button_release else gosub button_press
...
%---------------------
button_release:
	gr.modify g_press,"paint", p_backg
