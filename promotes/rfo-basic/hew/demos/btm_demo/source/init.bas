...
%---------------------------------------
 ...
 if scr_orient = -2 then		% auto fix orientation to detected
    ...
    gr.orientation scr_orient		% really hard fix it
 endif
 ...
 if scaling then
    ...
    row_h = scr_h / srows	  	% determine row height by srows
 else
    ...
    row_h = row_h*density/160		% correct for density
    srows = scr_h / row_h	   	% define #rows by row height
 endif

 txt_h = floor(row_h*5/6)		% insert a nice gap between text lines
%---------------------------------------
 ...
 bundle.put 1, "row_h", row_h
 bundle.put 1, "txt_h", txt_h
